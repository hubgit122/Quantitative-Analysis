...
/**
 * This Lexer must be created using LexerBuilder. It knows token and ignored terminals. To get this Lexer working the <i>setTerminals()</i> call must be called at least once. When using the Lexer standalone, the client must do this, else the Parser will call that method.
 * <p>
        ...
        Serializable
{
     ...
     *            list of Strings containing ignored symbols to scan. These are NOT enclosed in `backquotes` like tokens.
     * @param charConsumers
    ...
    private void setConsumers(List ignoredSymbols, Map charConsumers)
    {
        ...
        for (int i = 0; ignoredSymbols != null && i < ignoredSymbols.size(); i++)
    ...
    public void setTerminals(List terminals)
    {
        for (int i = 0; i < terminals.size(); i++)
        {
            ...
            if (ensureStrategy().hasTerminal(symbol) == false)
            { // could have been called for second time
                if (symbol.charAt(0) == Token.COMMAND_QUOTE)
                { // is a scan terminal covered by a Consumer
    ...
    /**
     * This is an optional functionality of Lexer. It is <b>NOT</b> called by the Parser. It can be used for heuristic reading from an input (not knowing if there is more input after the token was read).
     * <p />
     * The passed LexerSemantic will receive every matched rule (top-down) together with its ResultTree. See <i>lex()</i> for details.
     *
     * @param lexerSemantic
    ...
    /**
     * Implements Lexer: returns the next token from input, or EPSILON when no more input. This is called by the Parser to get the next syntax token from input. When returned <i>token.symbol</i> is null, no input could be recognized (ERROR).
     *
     * @param expectedTokenSymbols
     *            contains the expected String token symbols (in keys), can be null when no Parser drives this Lexer.
     * @return a Token with a terminal symbol and its instance text, or a Token with null symbol for error.
     */
    @Override
    ...
    /**
     * This is an optional functionality of Lexer. It is <b>NOT</b> called by the Parser. It can be used to run a standalone Lexer with a LexerSemantic, processing a ready-scanned syntax tree. Other than with Parser Semantic no value stack is available for LexerSemantic, and all input will have been read when LexerSemantic is called with the built syntax tree.
     * <p />
     * The passed LexerSemantic will receive every matched rule (top-down) together with its results ResultTree, containing the range within input. ResultTree can be converted to text by calling <i>resultTree.toString()</i>.
     * <p />
     ...
     * <p />
     * <b>MIND:</b> This method does not call any TokenListener, as the LexerSemantic is expected to dispatch results!
     *
     * @param lexerSemantic
     *            the LexerSemantic to be called with every evaluated Rule and its lexing ResultTree.
