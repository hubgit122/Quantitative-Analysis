...
public class SqlserverUpdater extends Analyzer
{
    ...
    public void scan(Stock stock) throws IOException
    {
        for (int i = 0; i < stock.history.size(); i++)
        {
            ...
            try
            {
                accesser.update("INSERT INTO dayline VALUES (?,?,?,?,?,?,?)", new Object[] { stock.number, stock.name, data.date, data.getVal(0), data.getVal(1), data.getVal(2), data.getVal(3), data.getVal(4), data.getVal(5), data.getVal(6) });
            }
            catch (Exception e)
            {
                e.printStackTrace();
